...
class SumoEnvironment:
    ...
    def _get_state(self):
        ...
        for i, phase_str in enumerate(self.phases):
            ...
            for idx, char in enumerate(phase_str):
                if char == 'G' and idx < len(self.controlled_lanes):
    ...
    def step(self, action):
        ...
        if action != self.current_phase_index:
            ...
            for _ in range(TrainingConfig.YELLOW_TIME):
                traci.simulationStep()
                step_speeds.append(self._get_avg_speed())
        
        # 绿灯阶段
        traci.trafficlight.setRedYellowGreenState(self.tl_id, self.phases[action])
        self.current_phase_index = action
        for _ in range(TrainingConfig.MIN_GREEN_TIME):
            traci.simulationStep()
            step_speeds.append(self._get_avg_speed())
        
        done = traci.simulation.getTime() >= TrainingConfig.MAX_STEPS_PER_EPISODE or \
               traci.simulation.getMinExpectedNumber() <= 0
        
        # 返回：状态，奖励，是否结束，本步的平均速度
        return self._get_state(), self._compute_reward(), done, np.mean(step_speeds)
...
def run_training():
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        # 增加 Avg_Speed 到表头
        writer.writerow(['Episode', 'Reward', 'Avg_Queue', 'Avg_Speed', 'Avg_Loss', 'Avg_Q_Value', 'Epsilon'])
    ...
    for episode in range(1, TrainingConfig.TOTAL_EPISODES + 1):
        ...
        ep_reward, ep_queue, ep_loss, ep_q, ep_speed = 0, [], [], [], []
        
        done = False
        while not done:
            action = agent.select_action(state)
            next_state, reward, done, step_avg_speed = env.step(action)
            ...
            ep_speed.append(step_avg_speed) # 记录速度
        ...
        with open(csv_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([episode, ep_reward, avg_q_len, avg_speed, avg_loss, avg_q_val, agent.epsilon])

        print(f"Ep {episode} | Reward: {ep_reward:.1f} | Queue: {avg_q_len:.2f} | Speed: {avg_speed:.2f} m/s | Loss: {avg_loss:.4f} | Q: {avg_q_val:.2f}")
